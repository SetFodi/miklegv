...
The private, non-technical content manager lives at `/admin`. Once connected to Supabase, Mikle can sign in with an approved email and password and manage videos, thumbnails, publishing, ordering, every visible bilingual label and message, metadata, the six health pillars, his biography, profile identity, images, logo, and account password. See [`CMS_SETUP.md`](./CMS_SETUP.md) for the one-time setup.
